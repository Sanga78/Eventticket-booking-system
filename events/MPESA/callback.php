...
$stmt = $conn->query("SELECT * FROM pays ORDER BY ID DESC LIMIT 1");
$stmt->execute();
...
foreach($rows as $row){
	...
	if($res['Body']['stkCallback']['ResultCode'] == '1032'){
		$sql = $conn->query("UPDATE `pays` SET `Status` = 'CANCELLED' WHERE `schedule_id`.`ID` = $ID");
		$rs = $sql->execute();
	 }else{
		$sql = $conn->query("UPDATE `pays` SET `Status` = 'SUCCESS' WHERE `schedule_id`.`ID` = $ID");
		$rs = $sql->execute();
	 }
